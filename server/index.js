...
app.get('/api/projects/:projectName/sessions/:sessionId/messages/new', authenticateToken, async (req, res) => {
    try {
        const { projectName, sessionId } = req.params;
        const { afterCount } = req.query;

        const parsedAfterCount = afterCount ? parseInt(afterCount, 10) : 0;
        const result = await getNewSessionMessages(projectName, sessionId, parsedAfterCount);
...
async function startServer() {
    try {
        ...
        // Slave mode: only listen on localhost (accessed via tunnel)
        // Master/Standalone: listen on all interfaces
        const listenHost = DEPLOYMENT_MODE === 'slave' ? '127.0.0.1' : '0.0.0.0';
        ...
        server.listen(PORT, listenHost, async () => {
            ...
            console.log(c.dim('═'.repeat(63)));
            console.log('');
            console.log(`${c.info('[INFO]')} Server URL:  ${c.bright('http://' + listenHost + ':' + PORT)}`);
